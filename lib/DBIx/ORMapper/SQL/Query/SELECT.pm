package DBIx::ORMapper::SQL::Query::SELECT;
...
use DBIx::ORMapper::SQL::Query::SELECT::Join;
use DBIx::ORMapper::SQL::Query::SELECT::Order;
...
# Function: new
#
#   Creates the DBIx::ORMapper::SQL::Query::SELECT Object.
#
# Returns:
#
#   DBIx::ORMapper::SQL::Query::SELECT
sub new {
   my $that = shift;
...
sub join {
   ...
   my $join = DBIx::ORMapper::SQL::Query::SELECT::Join->new( query => $self, __table => $_[0] );
...
# Returns:
#
#   DBIx::ORMapper::SQL::Query::SELECT::Order
sub order {
   my $self = shift;
   my @order_by = @_;

   $self->{'__order'} = DBIx::ORMapper::SQL::Query::SELECT::Order->new( query => $self, order_by => \@order_by );
   return $self->{'__order'};
}
